...
def main():
    ...
    for e in events:
        if list(g.objects(e, TS.countryLabel)):
            raw_presence["raw_country"] += 1
        if list(g.objects(e, TS.rawEventStatusLabel)):
            raw_presence["raw_status"] += 1
        if list(g.objects(e, TS.hasDisease)):
            norm_presence["disease"] += 1
        if list(g.objects(e, TS.hasEventStatus)):
            norm_presence["event_status"] += 1
        if list(g.objects(e, TS.belongsToSituation)):
            norm_presence["situation_link"] += 1

    # species via event hasSpecies
    for e in events:
        if list(g.objects(e, TS.hasSpecies)):
            norm_presence["species"] += 1
    ...
    for e in events:
        ...
        if (
            lat is not None
            and lon is not None
            and list(g.objects(e, TS.belongsToSituation))
        ):
    ...
    (out / "adis-rdf-qa-report.json").write_text(
